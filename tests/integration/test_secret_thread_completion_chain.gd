...
func test_completed_emits_secret_thread_completed_once_with_correct_args() -> void:
	...
	assert_eq(received_thread_id[0], THREAD_ID, "emitted thread_id must match")
	assert_eq(received_unlock_id[0], UNLOCK_ID, "emitted reward_unlock_id must match")
...
func test_completed_emits_unlock_granted_once() -> void:
	...
	_drive_complete_thread()
	assert_eq(emit_count[0], 1, "unlock_granted should emit exactly once for the reward unlock")


func test_failed_thread_emits_failed_and_no_unlock() -> void:
	_setup_thread(_fail_def)
	var failed_emitted: Array = [false]
	...
	for day: int in range(1, 4):
		_thread_system._on_day_started(day)
	assert_true(failed_emitted[0], "secret_thread_failed should be emitted on timeout")
	assert_eq(unlock_count[0], 0, "unlock_granted must NOT emit on failure")
	assert_false(
		_unlock_system.is_unlocked(UNLOCK_ID),
...
func test_unlock_granted_not_re_emitted_after_save_load_cycle() -> void:
	...
	_drive_complete_thread()
	assert_eq(emit_count[0], 1, "unlock_granted emitted once on first completion")
	...
	assert_eq(emit_count[0], 1, "unlock_granted must not re-emit after save/load re-evaluation")
	assert_true(
		_unlock_system.is_unlocked(UNLOCK_ID),
