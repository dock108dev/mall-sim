...
func test_scenario_a_satisfied_customers_accumulate_to_reputable() -> void:
	var tier_unremarkable: float = ReputationSystem.TIER_THRESHOLDS[
	...
	var needed: int = ceili(gap / ReputationSystem.SATISFACTION_GAIN) + 1
...
func test_scenario_b_reputation_score_decreases_by_daily_decay() -> void:
	...
	assert_almost_eq(
		...
		score_before - ReputationSystem.DAILY_DECAY, 0.01,
...
func test_scenario_c_dissatisfied_customer_decreases_score() -> void:
	...
	assert_almost_eq(
		_rep.get_reputation(STORE_ID),
		score_before + ReputationSystem.DISSATISFACTION_LOSS, 0.01,
...
func test_scenario_c_repeated_dissatisfied_customers_compound() -> void:
	...
	var expected: float = score_before + float(emit_count) * ReputationSystem.DISSATISFACTION_LOSS
	assert_almost_eq(
