...
func test_maybe_show_hint_day_start_emits_signal() -> void:
	...
	assert_eq(received_id, &"hint_day_start", "Should emit hint_id for day_start trigger")
	assert_false(received_msg.is_empty(), "Message should be non-empty")
	assert_false(received_pos.is_empty(), "Position hint should be non-empty")
...
func test_maybe_show_hint_emits_signal_for_known_trigger() -> void:
	...
	assert_eq(received_id, &"hint_store_entered", "Should emit hint_id for store_entered trigger")
	assert_false(received_msg.is_empty(), "Message should be non-empty")
	assert_false(received_pos.is_empty(), "Position hint should be non-empty")
...
func test_maybe_show_hint_does_not_emit_for_unknown_trigger() -> void:
	...
	assert_false(hint_emitted, "Should not emit for unknown trigger")
...
func test_maybe_show_hint_day_start_idempotent() -> void:
	...
	assert_eq(emit_count, 1, "day_start hint should emit only on first call")
...
func test_hint_not_re_emitted_after_first_showing() -> void:
	...
	assert_eq(emit_count, 1, "Hint should only emit once")
...
func test_no_hint_emitted_when_all_shown() -> void:
	...
	assert_false(hint_emitted, "Should not emit when all hints have already been shown")
...
func test_maybe_show_hint_silent_when_day_greater_than_one() -> void:
	...
	assert_false(hint_emitted, "Should not emit hints after Day 1")
...
func test_maybe_show_hint_silent_when_disabled() -> void:
	...
	assert_false(hint_emitted, "Should not emit hints when disabled")
...
func test_hint_bottom_left_position_emitted_correctly() -> void:
	...
	assert_eq(received_pos, "bottom_left", "Signal should carry correct position_hint")
...
func test_load_state_with_empty_dict_behaves_as_fresh_start() -> void:
	...
	assert_true(hint_emitted, "Should emit hint after loading empty state (fresh start)")
...
func test_load_data_restores_shown_hints() -> void:
	...
	assert_false(hint_emitted, "Hint should not re-emit after loading shown state")
