...
func test_flag_discrepancy_increments_counter_once_per_sku() -> void:
	var first: bool = _controller.audit.flag_discrepancy(&"retro_test_a", 5, 3)
	var second: bool = _controller.audit.flag_discrepancy(&"retro_test_a", 5, 3)
	assert_true(first, "First flag must record")
	assert_false(second, "Repeat flag for same SKU must be no-op")
	assert_eq(
		_controller.audit.get_discrepancies_flagged_today(), 1,
...
func test_flag_discrepancy_emits_inventory_variance_noted_once() -> void:
	...
	_controller.audit.flag_discrepancy(&"retro_test_b", 10, 7)
	_controller.audit.flag_discrepancy(&"retro_test_b", 10, 7)
...
func test_flag_discrepancy_distinct_skus_each_increment_counter() -> void:
	_controller.audit.flag_discrepancy(&"retro_test_c", 5, 4)
	_controller.audit.flag_discrepancy(&"retro_test_d", 8, 2)
	assert_eq(
		_controller.audit.get_discrepancies_flagged_today(), 2,
		"Two distinct SKUs must each register against the counter",
	)


func test_can_flag_discrepancy_returns_false_after_flag() -> void:
	assert_true(_controller.audit.can_flag_discrepancy(&"retro_test_e"))
	_controller.audit.flag_discrepancy(&"retro_test_e", 4, 2)
	assert_false(
		_controller.audit.can_flag_discrepancy(&"retro_test_e"),
		"can_flag_discrepancy must short-circuit after the SKU is flagged",
	)


func test_day_started_clears_flagged_set() -> void:
	_controller.audit.flag_discrepancy(&"retro_test_f", 6, 5)
	assert_eq(_controller.audit.get_discrepancies_flagged_today(), 1)
	EventBus.day_started.emit(2)
	assert_eq(
		_controller.audit.get_discrepancies_flagged_today(), 0,
		"day_started must reset the discrepancy counter",
	)
	assert_true(
		_controller.audit.can_flag_discrepancy(&"retro_test_f"),
		"A new day re-enables flagging the same SKU",
	)


func test_flag_discrepancy_rejects_empty_item_id() -> void:
	var ok: bool = _controller.audit.flag_discrepancy(&"", 1, 0)
	assert_false(ok, "Empty item_id must be rejected and not increment")
	assert_eq(_controller.audit.get_discrepancies_flagged_today(), 0)


func test_get_flagged_skus_today_lists_flagged_only() -> void:
	_controller.audit.flag_discrepancy(&"retro_test_g", 4, 1)
	_controller.audit.flag_discrepancy(&"retro_test_h", 2, 0)
	var keys: Array = _controller.audit.get_flagged_skus_today()
