...
func test_day_close_press_emits_day_close_requested_unconditionally() -> void:
	# Loop-completion gating (was: HUD/MallOverview Day-1 soft confirm dialog)
	# now lives downstream in DayCycleController + CloseDayConfirmationPanel
	# (see test_day_close_confirmation_gate.gd). MallOverview's job is just
	# to emit day_close_requested when the button is pressed.
	_overview = load(MALL_OVERVIEW_SCENE_PATH).instantiate() as MallOverview
	add_child_autofree(_overview)
	...
	assert_eq(
		_day_close_requests,
		1,
		"Day close press must emit day_close_requested without local gating"
	)
