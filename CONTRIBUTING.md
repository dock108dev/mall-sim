...
```
add customer browsing state machine
fix item tooltip position on ultrawide displays
remove unused shelf placement preview code
wire economy manager to end-of-day signal
```

If more context is needed, add a blank line then a body paragraph:

```
add haggling mechanic for collector customers

Collector-type customers now counter-offer at 80-90% of asking price.
Player can accept, reject, or counter. Rejection causes customer to
leave with a small reputation penalty.
```
...
Use the issue templates in `.github/` when filing bugs or feature requests. Include:

- **Bug:** steps to reproduce, expected behavior, actual behavior, Godot version
- **Feature:** what it should do, why it matters, which roadmap phase it belongs to
...
- **No premature optimization.** Write clear code first. Profile before optimizing. GDScript is fast enough for this game's logic.
...
- **No changes to autoload registration** without discussing in an issue first. Adding/removing autoloads affects the entire project.
- **No `print()` statements in committed code.** Use `push_warning()` or `push_error()` for diagnostics that should persist. Remove debug prints before committing.
- **No hardcoded magic numbers.** Use constants or config values. `if price > 100.0` should be `if price > HAGGLE_THRESHOLD`.
