## Mall-wide shopper NPC with personality-driven needs and utility-scored actions.
...
func initialize(spawn_position: Vector3) -> void:
	...
	if personality:
		shopper_budget = randf_range(
			personality.min_budget, personality.max_budget
		)
...
func _physics_full(delta: float) -> void:
	needs.update(
		delta, _is_moving_state(),
		_state_name(current_state), personality
	)
	...
	match current_state:
		...
		ShopperState.WINDOW_SHOPPING, ShopperState.EATING, \
		ShopperState.SITTING, ShopperState.SOCIALIZING:
...
func _physics_simple(delta: float) -> void:
	...
	match current_state:
		...
		ShopperState.WALKING:
			_process_walking(delta)
		ShopperState.BROWSING:
			_skip_browsing_to_buy_or_leave()
		ShopperState.WINDOW_SHOPPING, ShopperState.EATING, \
		ShopperState.SITTING, ShopperState.SOCIALIZING:
			_process_timed_state(delta)
		ShopperState.BUYING:
...
func _score_action(action: String) -> float:
	return needs.score_action(
		action, personality, _store_appeal(),
		_nav.waypoint_proximity(
			MallWaypoint.WaypointType.FOOD_COURT_SEAT
		),
		_nav.waypoint_proximity(MallWaypoint.WaypointType.BENCH),
		_nearby_shopper_count(), _time_spent_factor()
	)


func _evaluate_next_action() -> void:
	if current_state in [
		ShopperState.ENTERING, ShopperState.LEAVING,
		ShopperState.BUYING
	]:
		return
	if _state_timer > 0.0:
	...
	for action: String in actions:
		var s: float = _score_group_or_solo(action)
		if s > best_score:
			best_score = s
			best_action = action
...
func _navigate_to_store() -> void:
	var wp: MallWaypoint = _nav.get_nearest_of_type(
	...
	if not wp:
		...
		return
	_nav.navigate_to_waypoint(wp)
	_sync_target()
	_transition_to(ShopperState.WALKING)


func _navigate_to_type(
	wp_type: MallWaypoint.WaypointType
) -> void:
	var wp: MallWaypoint = _nav.get_nearest_of_type(wp_type)
	if not wp:
		_transition_to(ShopperState.WALKING)
		return
	_nav.set_target(wp)
...
func _process_walking(delta: float) -> void:
	...
	if not _nav.has_arrived_at_target():
		return
	var wp: MallWaypoint = target_waypoint
...
func _process_buying(delta: float) -> void:
	...
	if _state_timer <= 0.0:
		_made_purchase = true
		EventBus.customer_purchased.emit(&"", &"unknown_item", 0.0, &"")
		if not _nav.pick_exit_path():
			_despawn()
			return
		_sync_target()
	...
	match new_state:
		ShopperState.BROWSING:
			_state_timer = randf_range(
				BROWSE_TIME_MIN, BROWSE_TIME_MAX
			) * browse_mult
			_look_timer = LOOK_CYCLE_INTERVAL
		ShopperState.WINDOW_SHOPPING:
			_state_timer = randf_range(
				WINDOW_SHOP_TIME_MIN, WINDOW_SHOP_TIME_MAX
			)
		ShopperState.BUYING:
		...
		ShopperState.SOCIALIZING:
			_state_timer = randf_range(
				SOCIALIZE_TIME_MIN, SOCIALIZE_TIME_MAX
			)
		ShopperState.LEAVING:
			if _waypoint_agent.is_path_complete():
				if not _nav.pick_exit_path():
...
func _is_moving_state() -> bool:
	return (
		...
		or current_state == ShopperState.WALKING
		or current_state == ShopperState.LEAVING
	)
...
func _process_follower_movement(delta: float) -> void:
	var idx: int = _get_follower_index()
	if idx < 0:
		return
	if not shopper_group.leader or not is_instance_valid(
		shopper_group.leader
	):
		return
	current_state = shopper_group.leader.current_state
	var slot_pos: Vector3 = (
		shopper_group.get_formation_world_position(idx)
	)
	var diff: Vector3 = slot_pos - global_position
	diff.y = 0.0
	if diff.length_squared() < 0.01:
		velocity = Vector3.ZERO
		return
	var speed: float = ShopperNavigation.BASE_WALK_SPEED
	if shopper_group.should_follower_catch_up(idx):
		speed *= ShopperGroup.CATCHUP_SPEED_MULT
