...
func get_trend_multiplier(item: ItemInstance) -> float:
	...
	for trend: Dictionary in _active_trends:
		if not _item_matches_trend(item.definition, trend):
			continue
		var mult: float = _calc_trend_multiplier(trend, current_day)
		combined *= mult
...
func _calc_trend_multiplier(trend: Dictionary, day: int) -> float:
	...
	if day < active_day:
		return 1.0
	if day >= active_day and day < end_day:
		return multiplier
	if day >= end_day and day < fade_end:
		var fade_progress: float = (
			float(day - end_day) / float(maxi(fade_end - end_day, 1))
		)
