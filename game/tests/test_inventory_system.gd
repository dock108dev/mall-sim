...
func test_add_item_retrievable_via_get_stock() -> void:
	var def: ItemDefinition = _get_first_def()
	if not def:
		pass_test("No item definitions — skip")
		return
...
func test_remove_item_returns_true_and_removes() -> void:
	var def: ItemDefinition = _get_first_def()
	if not def:
		pass_test("No item definitions — skip")
		return
	...
	for s_item: ItemInstance in stock:
		assert_ne(
			s_item.instance_id, item.instance_id,
			"Removed item should not appear in get_stock()"
		)
...
func test_queue_restock_then_process_adds_items() -> void:
	var def: ItemDefinition = _get_first_def()
	if not def:
		pass_test("No item definitions — skip")
		return
