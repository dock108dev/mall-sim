...
func test_slot_assignment_emits_success_signal() -> void:
	...
	assert_eq(_lease_results[0]["store_id"], &"video_rental")
...
func test_set_active_store_with_unregistered_id_still_sets() -> void:
	_manager.set_active_store(&"nonexistent_store")
	assert_eq(
		_manager.active_store_id, &"nonexistent_store",
		"set_active_store accepts any StringName without validation error"
	)
...
func test_store_entered_fires_with_correct_id() -> void:
	...
	assert_eq(
		_store_entered[0], &"sports_memorabilia",
		"store_entered payload should match the entered store_id"
	)
...
func test_switching_stores_emits_both_signals() -> void:
	...
	assert_eq(_store_exited[0], &"retro_games")
	assert_eq(_store_entered.size(), 1)
	assert_eq(_store_entered[0], &"sports_memorabilia")
